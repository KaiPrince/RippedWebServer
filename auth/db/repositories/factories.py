...
from db.repositories.mongo import UsersMongoRepository
from db.repositories.sql import UsersSqlRepository
...
def make_users_sql_repo():
    db = get_sql_db()
    return UsersSqlRepository(db)


def make_users_mongo_repo():
    db = get_mongo_db()
    return UsersMongoRepository(db)
